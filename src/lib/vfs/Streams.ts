...
class ReadStream extends Readable {
   ...
   * @private
   */
  _destroy (e: Error, cb: (error?: Error | null | undefined) => void) {
    this._close((e_) => {
      cb(e || e_);
    });
  }
   ...
   * @private
   */
  _close (cb?: (error?: Error | null | undefined) => void) {
    if (cb) {
      super.once('close', cb);
    }
    if (typeof this.fd !== 'number') {
...
class WriteStream extends Writable {
  ...
  constructor (path: string, options: optionsStream, fs: VirtualFS) {
    ...
    if (options.encoding) {
      super.setDefaultEncoding(options.encoding as BufferEncoding);
    }
    if (typeof this.fd !== 'number') {
      this._open();
    }
