...
import type { INode } from './INodes.ts';
import type { optionsStream } from './Streams.ts';
...
import CurrentDirectory from './CurrentDirectory.ts';
import {
  DEFAULT_ROOT_UID,
  DEFAULT_ROOT_GID,
  DEFAULT_ROOT_PERM,
...
} from './FileDescriptors.ts';
import { ReadStream, WriteStream } from './Streams.ts';
import { VirtualFSError, errno } from './VirtualFSError.ts';
...
type callback = (arg: VirtualFSError|null, arg2?: any) => void;
type navigated = {
  dir: Directory,
  target: any|null,
  name: string,
...
class VirtualFS {
  ...
  appendFileSync (
    ...
    try {
      ...
      if (typeof file === 'number') {
        ...
        if (!fd) throw new VirtualFSError(errno.EBADF, undefined, undefined, 'appendFile');
        if (!(fd.getFlags() & (constants.O_WRONLY | constants.O_RDWR))) {
          throw new VirtualFSError(errno.EBADF, undefined, undefined, 'appendFile');
        }
      } else {
        [fd, fdIndex] = this._openSync(file, options.flag as string, options.mode);
      }
      try {
        fd.write(data, null, constants.O_APPEND);
      } catch (e) {
        if (e instanceof RangeError) {
          throw new VirtualFSError(errno.EFBIG, undefined, undefined, 'appendFile');
        }
        throw e;
      }
    } finally {
  ...
  chmodSync (path: path, mode: number): void {
    ...
    if (this._uid !== DEFAULT_ROOT_UID && this._uid !== targetMetadata.uid) {
      throw new VirtualFSError(errno.EPERM, undefined, undefined, 'chmod');
    }
  ...
  chownSync (path: path, uid: number, gid: number): void {
    ...
    if (this._uid !== DEFAULT_ROOT_UID) {
      ...
      if (targetMetadata.uid !== this._uid) {
        throw new VirtualFSError(errno.EPERM, undefined, undefined, 'chown');
      }
  ...
  createReadStream (path: path, options?: optionsStream): ReadStream {
    ...
    if (options.start !== undefined) {
      if (options.start > (options.end as number)) {
  ...
  fchmodSync (fdIndex: number, mode: number): void {
    ...
    fdMetadata.mode = (fdMetadata.mode & constants.S_IFMT) | mode;
    return;
  }
    ...
    while (true) {
      ...
      try {
        ...
        if (options.encoding === 'buffer') {
          ...
          return Buffer.from(pathS).toString(options.encoding as BufferEncoding);
        }
      } catch (e) {
    ...
    return [...navigated.target.getEntries()]
      ...
      .map(([name, _]) => {
        ...
        if (options.encoding === 'buffer') {
          ...
          return Buffer.from(name).toString(options.encoding as BufferEncoding);
        }
      });
  }
  ...
  readFileSync (file: file, options?: options): string|Buffer {
    ...
    try {
      ...
      if (typeof file === 'number') {
        ...
        fdIndex = this.openSync(file, options.flag as string);
        while (bytesRead !== 0) {
          bytesRead = this.readSync(fdIndex, buffer, 0, buffer.length);
          totalBuffer = Buffer.concat([totalBuffer, buffer.slice(0, bytesRead)]);
        }
      }
      return (options.encoding) ? totalBuffer.toString(options.encoding as BufferEncoding) : totalBuffer;
    } finally {
      if (fdIndex !== undefined) this.closeSync(fdIndex);
    }
  }
  ...
  readlinkSync (path: path, options?: options): string|Buffer {
    ...
    if (options.encoding === 'buffer') {
      ...
      return Buffer.from(link).toString(options.encoding as BufferEncoding);
    }
  }
  ...
  realpathSync (path: path, options?: options): string|Buffer {
    ...
    if (options.encoding === 'buffer') {
      ...
      return Buffer.from('/' + navigated.pathStack.join('/')).toString(options.encoding as BufferEncoding);
    }
  }
  ...
  renameSync (oldPath: path, newPath: path): void {
    ...
    if (navigatedTarget.target) {
      ...
      navigatedTarget.dir.addEntry(navigatedTarget.name, index as number);
    } else {
  ...
  writeFileSync (file: file, data: data = 'undefined', options?: options) {
    ...
    try {
      ...
      if (typeof file === 'number') {
        ...
        fdIndex = this.openSync(file, options.flag as string, options.mode);
  ...
  _parsePath (pathS: string): {segment: string, rest: string} {
    const matches: RegExpMatchArray | null = pathS.match(/^([\s\S]*?)(?:\/+|$)([\s\S]*)/);
    if (matches) {
